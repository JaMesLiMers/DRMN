...
from drmn.models.modeling import VISUAL_CONFIG, BertPreTrainedModel
from drmn.models.modeling import BertEmbeddings, CrossEncoder, BertPooler
...
class BertEncoder(nn.Module):
    ...
    def forward(self, sents):
        ...
        input_ids = torch.tensor([f.input_ids for f in train_features], dtype=torch.long, device=next(self.parameters()).device)
        input_mask = torch.tensor([f.input_mask for f in train_features], dtype=torch.long, device=next(self.parameters()).device)
        segment_ids = torch.tensor([f.segment_ids for f in train_features], dtype=torch.long, device=next(self.parameters()).device)
        #shape=[60,230]
