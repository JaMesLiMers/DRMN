# Adapted from preserved utils/pre_process.py; token/label algorithm unchanged.
import sys
import json
import torch
import argparse
...
parser.add_argument("--vocab", default="configs/bert/vocab.txt")
parser.add_argument("--splits", nargs="+", choices=["train2017", "val2017"], default=["train2017", "val2017"])
...
tokenizer = BertTokenizer(args.vocab, do_lower_case=True)
...
for split in tqdm(splits):
    ...
    save_json(
        osp.join(
            ann_dir, "png_coco_{}_dataloader.json".format(split),
        ),
        all_dict,
    )
