...
Required inputs are COCO 2017 images, panoptic JSON, RGB-encoded panoptic PNG masks, and `png_coco_*_dataloader.json` containing `image_id`, `caption`, `noun_vector`, `labels`, and `boxes`.

Run `python tools/encode_data.py --help` for path arguments. The tool loads either an intact combined checkpoint or separate pretrained FPN/BERT weights before exporting features. A supplied combined checkpoint also undergoes strict DRMN head loading. State validation checks parameter compatibility and finite values; it does not establish whether weights were pretrained. Use the documented encoder sources. `noun_vector` must align with the original BERT WordPiece tokens; truncation that discards annotations raises an error. All valid records are processed by default. Use `--limit` for an explicit smaller check.
...
Synthetic fixtures are available for pipeline checks; they do not measure model accuracy.
